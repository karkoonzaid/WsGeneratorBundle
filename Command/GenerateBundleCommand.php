...
use Ws\Bundle\GeneratorBundle\Generator\BundleGenerator;
use Ws\Bundle\GeneratorBundle\Manipulator\KernelManipulator;
...
class GenerateBundleCommand extends GeneratorCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp(<<<EOT
The <info>ws:generate:bundle</info> command helps you generates new bundles.
...
<info>php app/console ws:generate:bundle --namespace=Ws/BlogBundle</info>
...
<info>php app/console ws:generate:bundle --namespace=Ws/BlogBundle --dir=src [--bundle-name=...] --no-interaction</info>

Note that the bundle namespace must end with "Bundle".
EOT
            )
            ->setName('ws:generate:bundle')
            ->setAliases(array('ck:generate:bundle'))
        ;
    }
    ...
    {
        $dialog = $this->getDialogHelper();
        $dialog->writeSection($output, 'Welcome to the WsGeneratorBundle Bundle generator');
        ...
        if (null === $namespace) {
            $output->writeln(array(
                ...
                '',
                'Each bundle is hosted under a namespace (like <comment>Ws/Bundle/BlogBundle</comment>).',
            ...
            $namespace = $dialog->askAndValidate($output, $dialog->getQuestion('Bundle namespace', $input->getOption('namespace')), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateBundleNamespace'), false, $input->getOption('namespace'));
        ...
        if (null === $bundle) {
            ...
            $bundle = $dialog->askAndValidate($output, $dialog->getQuestion('Bundle name', $bundle), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateBundleName'), false, $bundle);
        ...
        if (null === $format) {
            ...
            $format = $dialog->askAndValidate($output, $dialog->getQuestion('Configuration format (yml, xml, php, or annotation)', $input->getOption('format')), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateFormat'), false, $input->getOption('format'));

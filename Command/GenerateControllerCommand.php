...
use Ws\Bundle\GeneratorBundle\Generator\ControllerGenerator;
...
class GenerateControllerCommand extends GeneratorCommand
{
    ...
    public function configure()
    {
        $this
            ...
            ->setHelp(<<<EOT
The <info>ws:generate:controller</info> command helps you generates new controllers
inside bundles.
...
<info>php app/console ws:generate:controller --controller=WsBlogBundle:Post</info>

If you want to disable any user interaction, use <comment>--no-interaction</comment>
but don't forget to pass all needed options:

<info>php app/console ws:generate:controller --controller=WsBlogBundle:Post --no-interaction</info>
...
<info>BUNDLE_PATH/Resources/WsGeneratorBundle/skeleton/controller
APP_PATH/Resources/WsGeneratorBundle/skeleton/controller</info>

You can check https://github.com/web-solution/WsGeneratorBundle/tree/master/Resources/skeleton
in order to know the file structure of the skeleton
EOT
            )
            ->setName('ws:generate:controller')
            ->setAliases(array('ck:generate:controller'))
        ;
    }
    ...
    {
        $dialog = $this->getDialogHelper();
        $dialog->writeSection($output, 'Welcome to the WsGeneratorBundle controller generator');
        ...
        while (true) {
            $controller = $dialog->askAndValidate($output, $dialog->getQuestion('Controller name', $input->getOption('controller')), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateControllerName'), false, $input->getOption('controller'));
        ...
        $routeFormat = $dialog->askAndValidate($output, $dialog->getQuestion('Routing format (php, xml, yml, annotation)', $defaultFormat), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateFormat'), false, $defaultFormat);
    ...
    public function parseShortcutNotation($shortcut)
    {
        ...
        if (false === $pos = strpos($entity, ':')) {
            throw new \InvalidArgumentException(sprintf('The controller name must contain a : ("%s" given, expecting something like WsBlogBundle:Post)', $entity));
        }

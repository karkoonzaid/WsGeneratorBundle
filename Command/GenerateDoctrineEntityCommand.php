...
use Ws\Bundle\GeneratorBundle\Generator\DoctrineEntityGenerator;
use Ws\Bundle\GeneratorBundle\Command\Helper\DialogHelper;
...
class GenerateDoctrineEntityCommand extends GenerateDoctrineCommand
{
    ...
    {
        $this
            ->setName('ws:generate:entity')
            ->setAliases(array('ck:generate:entity'))
            ...
            ->setHelp(<<<EOT
The <info>ws:generate:entity</info> task generates a new Doctrine
entity inside a bundle:

<info>php app/console ws:generate:entity --entity=WsBlogBundle:Blog/Post</info>

The above command would initialize a new entity in the following entity
namespace <info>Ws\BlogBundle\Entity\Blog\Post</info>.

You can also optionally specify the fields you want to generate in the new
entity:

<info>php app/console ws:generate:entity --entity=WsBlogBundle:Blog/Post --fields="title:string(255) body:text"</info>

The command can also generate the corresponding entity repository class with the
<comment>--with-repository</comment> option:

<info>php app/console ws:generate:entity --entity=WsBlogBundle:Blog/Post --with-repository</info>

By default, the command uses annotations for the mapping information; change it
with <comment>--format</comment>:

<info>php app/console ws:generate:entity --entity=WsBlogBundle:Blog/Post --format=yml</info>

To deactivate the interaction mode, simply use the `--no-interaction` option
without forgetting to pass all needed options:

<info>php app/console ws:generate:entity --entity=WsBlogBundle:Blog/Post --format=annotation --fields="title:string(255) body:text" --with-repository --no-interaction</info>
EOT
        );
    }
    ...
    {
        $dialog = $this->getDialogHelper();
        $dialog->writeSection($output, 'Welcome to the WsGenerateBundle entity generator');
        ...
        $output->writeln(array(
            ...
            'You must use the shortcut notation like <comment>WsBlogBundle:Post</comment>.',
            ''
        ));
        ...
        while (true) {
            $entity = $dialog->askAndValidate($output, $dialog->getQuestion('The Entity shortcut name', $input->getOption('entity')), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateEntityName'), false, $input->getOption('entity'), $bundleNames);
        ...
        $format = $dialog->askAndValidate($output, $dialog->getQuestion('Configuration format (yml, xml, php, or annotation)', $input->getOption('format')), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateFormat'), false, $input->getOption('format'), $formats);

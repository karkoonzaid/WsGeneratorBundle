...
use Ws\Bundle\GeneratorBundle\Generator\DoctrineCrudGenerator;
use Ws\Bundle\GeneratorBundle\Generator\DoctrineFormGenerator;
use Ws\Bundle\GeneratorBundle\Manipulator\RoutingManipulator;
...
class GenerateDoctrineCrudCommand extends GenerateDoctrineCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setHelp(<<<EOT
The <info>ws:generate:crud</info> command generates a CRUD based on a Doctrine entity.

The default command only generates the list and show actions.

<info>php app/console ws:generate:crud --entity=WsBlogBundle:Post --route-prefix=post_admin</info>

Using the --with-write option allows to generate the new, edit and delete actions.

<info>php app/console ws:generate:crud --entity=WsBlogBundle:Post --route-prefix=post_admin --with-write</info>

Every generated file is based on a template. There are default templates but they can be overriden by placing custom templates in one of the following locations, by order of priority:

<info>BUNDLE_PATH/Resources/WsGeneratorBundle/skeleton/crud
APP_PATH/Resources/WsGeneratorBundle/skeleton/crud</info>

And

<info>__bundle_path__/Resources/WsGeneratorBundle/skeleton/form
__project_root__/app/Resources/WsGeneratorBundle/skeleton/form</info>

You can check https://github.com/web-solution/WsGeneratorBundle/tree/master/Resources/skeleton
in order to know the file structure of the skeleton
EOT
            )
            ->setName('ws:generate:crud')
            ->setAliases(array('ck:generate:crud'))
        ;
    }
    ...
    {
        $dialog = $this->getDialogHelper();
        $dialog->writeSection($output, 'Welcome to the WsGeneratorBundle CRUD generator');
        ...
        $entity = $dialog->askAndValidate($output, $dialog->getQuestion('The Entity shortcut name', $input->getOption('entity')), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateEntityName'), false, $input->getOption('entity'));
        ...
        $format = $dialog->askAndValidate($output, $dialog->getQuestion('Configuration format (yml, xml, php, or annotation)', $format), array('Ws\Bundle\GeneratorBundle\Command\Validators', 'validateFormat'), false, $format);
